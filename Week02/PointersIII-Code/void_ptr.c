...
int main() {
   ...
   v_ptr = &int_value; /* Assigning to void pointer variable a pointer to int */
   v_ptr = &float_value; /* Assigning to void pointer variable a pointer to float */

   /* printf("%f\n", *v_ptr); We cannot dereference a void ptr */
   printf("V1: %f\n", *(float *)v_ptr); /* Casting void ptr */

   /* Assigning a void pointer value to an integer/float etc. pointer variable */
   f_ptr = v_ptr;  /* No casting needed */
   printf("V2: %f\n", *f_ptr); 
